...
errorcode_t mesh_lib_init(void *(*malloc_fn)(size_t), void (*free_fn)(void *),
...
errorcode_t mesh_lib_generic_server_register_handler(uint16_t model_id,
		uint16_t elem_index, mesh_lib_generic_server_client_request_cb cb,
...
errorcode_t mesh_lib_generic_client_register_handler(uint16_t model_id,
		uint16_t elem_index, mesh_lib_generic_client_server_response_cb cb) {
	struct reg *reg = NULL;
...
void mesh_lib_generic_server_event_handler(struct gecko_cmd_packet *evt) {
	...
	case gecko_evt_mesh_generic_server_client_request_id:
		...
		if (reg) {
			if (mesh_lib_deserialize_request(&request, req->type,
					req->parameters.data, req->parameters.len) == 0) {
				(reg->server.client_request_cb)(req->model_id, req->elem_index,
						req->client_address, req->server_address,
						req->appkey_index, &request, req->transition,
						req->delay, req->flags);
			}
		}
		break;
	case gecko_evt_mesh_generic_server_state_changed_id:
		...
		if (reg) {
			if (mesh_lib_deserialize_state(&current, &target, &has_target,
					chg->type, chg->parameters.data, chg->parameters.len)
					== 0) {
				(reg->server.state_changed_cb)(chg->model_id, chg->elem_index,
						&current, has_target ? &target : NULL, chg->remaining);
			}
		}
		break;
	}
...
void mesh_lib_generic_client_event_handler(struct gecko_cmd_packet *evt) {
	...
	case gecko_evt_mesh_generic_client_server_status_id:
		...
		if (reg) {
			if (mesh_lib_deserialize_state(&current, &target, &has_target,
					res->type, res->parameters.data, res->parameters.len)
					== 0) {
				(reg->client.server_response_cb)(res->model_id, res->elem_index,
						res->client_address, res->server_address, &current,
						has_target ? &target : NULL, res->remaining,
						res->flags);
			}
		}
		break;
...
errorcode_t mesh_lib_generic_server_response(uint16_t model_id,
		uint16_t element_index, uint16_t client_addr, uint16_t appkey_index,
	...
	if (mesh_lib_serialize_state(current, target, buf, sizeof(buf), &len)
			!= 0) {
		return bg_err_invalid_param;
	}
	return gecko_cmd_mesh_generic_server_response(model_id, element_index,
			client_addr, appkey_index, remaining_ms, response_flags,
			current->kind, len, buf)->result;
}

errorcode_t mesh_lib_generic_server_update(uint16_t model_id,
		uint16_t element_index, const struct mesh_generic_state *current,
		const struct mesh_generic_state *target, uint32_t remaining_ms) {
	uint8_t buf[12];
	size_t len;
	...
	return gecko_cmd_mesh_generic_server_update(model_id, element_index,
			remaining_ms, current->kind, len, buf)->result;
}

errorcode_t mesh_lib_generic_server_publish(uint16_t model_id,
		uint16_t element_index, mesh_generic_state_t kind) {
	return gecko_cmd_mesh_generic_server_publish(model_id, element_index, kind)->result;
}

errorcode_t mesh_lib_generic_client_get(uint16_t model_id,
		uint16_t element_index, uint16_t server_addr, uint16_t appkey_index,
		mesh_generic_state_t kind) {
	return gecko_cmd_mesh_generic_client_get(model_id, element_index,
			server_addr, appkey_index, kind)->result;
}

errorcode_t mesh_lib_generic_client_set(uint16_t model_id,
		uint16_t element_index, uint16_t server_addr, uint16_t appkey_index,
		uint8_t transaction_id, const struct mesh_generic_request *request,
		uint32_t transition_ms, uint16_t delay_ms, uint8_t flags) {
	uint8_t buf[10];
	size_t len;
	...
	return gecko_cmd_mesh_generic_client_set(model_id, element_index,
			server_addr, appkey_index, transaction_id, transition_ms, delay_ms,
			flags, request->kind, len, buf)->result;
}

errorcode_t mesh_lib_generic_client_publish(uint16_t model_id,
		uint16_t element_index, uint8_t transaction_id,
	...
	return gecko_cmd_mesh_generic_client_publish(model_id, element_index,
			transaction_id, transition_ms, delay_ms, request_flags,
			request->kind, len, buf)->result;
}
