...
static const struct hfrcoCtrlTableElement {
	...
	CMU_HFRCOFreq_TypeDef band;
		...
		{ 860000UL, 1050000UL, 0xBC601F00UL, cmuHFRCOFreq_1M0Hz }, { 1050000UL,
				1280000UL, 0xBC611F35UL, (CMU_HFRCOFreq_TypeDef) 0 }, {
		...
		{ 3600000UL, 4220000UL, 0xB8601F00UL, cmuHFRCOFreq_4M0Hz }, { 4220000UL,
				5120000UL, 0xB8611F35UL, (CMU_HFRCOFreq_TypeDef) 0 }, {
				5120000UL, 5930000UL, 0xB8A21F35UL, (CMU_HFRCOFreq_TypeDef) 0 },
		{ 5930000UL, 7520000UL, 0xA9231F00UL, cmuHFRCOFreq_7M0Hz }, { 7520000UL,
				9520000UL, 0x99241F35UL, (CMU_HFRCOFreq_TypeDef) 0 },
...
static void setHfLeConfig(uint32_t hfFreq) {
	...
	GENERIC_HFLE_PRESC_REG =
			(GENERIC_HFLE_PRESC_REG & ~_GENERIC_HFLE_PRESC_MASK)
...
static uint32_t lfClkGet(CMU_Clock_TypeDef lfClkBranch) {
	...
	case cmuClock_LFA:
		...
		sel = (CMU->LFACLKSEL & _CMU_LFACLKSEL_LFA_MASK)
	...
	case cmuClock_LFB:
		...
		sel = (CMU->LFBCLKSEL & _CMU_LFBCLKSEL_LFB_MASK)
...
#if defined(_CMU_LFECLKSEL_MASK)
	case cmuClock_LFE:
		sel = (CMU->LFECLKSEL & _CMU_LFECLKSEL_LFE_MASK)
...
uint32_t CMU_Calibrate(uint32_t HFCycles, CMU_Osc_TypeDef reference) {
	EFM_ASSERT(
			HFCycles <= (_CMU_CALCNT_CALCNT_MASK >> _CMU_CALCNT_CALCNT_SHIFT));
...
void CMU_CalibrateConfig(uint32_t downCycles, CMU_Osc_TypeDef downSel,
	...
	EFM_ASSERT(
			downCycles <= (_CMU_CALCNT_CALCNT_MASK >> _CMU_CALCNT_CALCNT_SHIFT));
...
#else
		ret /= CMU_Log2ToDiv(
				(CMU->LFAPRESC0 & _CMU_LFAPRESC0_LETIMER0_MASK)
						>> _CMU_LFAPRESC0_LETIMER0_SHIFT);
#endif
		break;
#endif
...
bool CMU_DPLLLock(const CMU_DPLLInit_TypeDef *init) {
	...
	EFM_ASSERT(
			init->frequency <= hfrcoCtrlTable[HFRCOCTRLTABLE_ENTRIES - 1U].maxFreq);
	EFM_ASSERT(init->n > 300U);
	...
	EFM_ASSERT(
			init->ssInterval <= (_CMU_HFRCOSS_SSINV_MASK >> _CMU_HFRCOSS_SSINV_SHIFT));
	EFM_ASSERT(
			init->ssAmplitude <= (_CMU_HFRCOSS_SSAMP_MASK >> _CMU_HFRCOSS_SSAMP_SHIFT));
	...
	if (hfrcoCtrlTable[index].band != (CMU_HFRCOFreq_TypeDef) 0) {
		...
		tuning = (CMU_HFRCODevinfoGet(hfrcoCtrlTable[index].band)
				& _CMU_HFRCOCTRL_TUNING_MASK) >> _CMU_HFRCOCTRL_TUNING_SHIFT;
	...
	while ((lockStatus =
			(CMU->IF
					& (CMU_IF_DPLLRDY | CMU_IF_DPLLLOCKFAILLOW
...
bool enEM0EM1StartSel) {
	...
	if (enEM0EM1StartSel) {
		...
		EFM_ASSERT(
				(EMU->CTRL & _EMU_CTRL_EM23VSCALE_MASK) == EMU_CTRL_EM23VSCALE_VSCALE2);
	}
#endif
	...
	hfxoCtrl = CMU->HFXOCTRL
			& ~(userSel | _CMU_HFXOCTRL_AUTOSTARTEM0EM1_MASK
					| _CMU_HFXOCTRL_AUTOSTARTSELEM0EM1_MASK);

	hfxoCtrl |= userSel | (enEM0EM1Start ? CMU_HFXOCTRL_AUTOSTARTEM0EM1 : 0UL)
...
void CMU_HFXOInit(const CMU_HFXOInit_TypeDef *hfxoInit) {
  ...
  && (_SILICON_LABS_GECKO_INTERNAL_SDID >= 100)
	...
	EFM_ASSERT(
			!(hfxoInit->autoStartEm01 || hfxoInit->autoSelEm01
	...
	_CMU_HFXOCTRL_LOWPOWER_SHIFT, (unsigned) hfxoInit->lowPowerMode);
...
#if defined(_SILICON_LABS_32B_SERIES_1)
	uint32_t reg = (CMU->LFXOCTRL
			& ~(_CMU_LFXOCTRL_TUNING_MASK | _CMU_LFXOCTRL_GAIN_MASK
					| _CMU_LFXOCTRL_TIMEOUT_MASK | _CMU_LFXOCTRL_MODE_MASK))
			...
			if ((osc == cmuOsc_HFXO) && firstHfxoEnable) {
				if ((CMU->HFXOCTRL & _CMU_HFXOCTRL_MODE_MASK)
					...
					/* Wait for the peak detection and shunt current optimization
					 to complete. */
					(void) CMU_OscillatorTuningWait(cmuOsc_HFXO,
...
void CMU_OscillatorTuningSet(CMU_Osc_TypeDef osc, uint32_t val) {
	...
	case cmuOsc_AUXHFRCO:
		...
		val &=
				(_CMU_AUXHFRCOCTRL_TUNING_MASK >> _CMU_AUXHFRCOCTRL_TUNING_SHIFT);
#if defined(_SILICON_LABS_32B_SERIES_1)
		while (BUS_RegBitRead(&CMU->SYNCBUSY,
		_CMU_SYNCBUSY_AUXHFRCOBSY_SHIFT) != 0UL) {
		}
...
#if defined(_CMU_HFXOSTEADYSTATECTRL_REGISH_MASK)
		regIshUpper = getRegIshUpperVal(
				(val & _CMU_HFXOSTEADYSTATECTRL_REGISH_MASK)
						>> _CMU_HFXOSTEADYSTATECTRL_REGISH_SHIFT);
		CMU->HFXOSTEADYSTATECTRL = (CMU->HFXOSTEADYSTATECTRL
				& ~(_CMU_HFXOSTEADYSTATECTRL_IBTRIMXOCORE_MASK
						| _CMU_HFXOSTEADYSTATECTRL_REGISH_MASK
						| _CMU_HFXOSTEADYSTATECTRL_REGISHUPPER_MASK)) | val
				| regIshUpper;
#else
		CMU->HFXOSTEADYSTATECTRL = (CMU->HFXOSTEADYSTATECTRL
				& ~_CMU_HFXOSTEADYSTATECTRL_IBTRIMXOCORE_MASK)
